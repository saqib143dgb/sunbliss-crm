...
function specialLifecycle(note,schedules,credits){
  var n=norm(note);
  if(!n||n.indexOf('credit note')<0)return {creditRelated:false,multi:false,completed:false};
  schedules=schedules||[];
  credits=credits||[];
  var credited={};
  ...
  if(multi){
    var priceStages=schedules.filter(isPriceStage);
    return {creditRelated:true,multi:true,completed:priceStages.length>0&&priceStages.every(function(s){return !!credited[String(s.id)];})};
  }
  ...
  if(named.length){
    return {creditRelated:true,multi:false,completed:named.some(function(s){return !!credited[String(s.id)];})};
  }
  return {creditRelated:true,multi:false,completed:credits.length>0};
}
async function getState(uid,force){
  var now=Date.now();
  var c=cache[String(uid)];
  if(!force&&c&&now-c.at<1500)return c.data;
  var results=await Promise.all([
    sb.from('sales').select('remarks').eq('unit_id',uid).order('id',{ascending:false}).limit(1),
    sb.from('payment_schedule').select('id,stage_name,due_amount').eq('unit_id',uid),
  ...
  var note=text(sale.remarks).trim();
  var life=specialLifecycle(note,results[1].data||[],results[2].data||[]);
  var data={note:note,life:life};
  cache[String(uid)]={at:now,data:data};
  return data;
}
function findSpecialRow(root){
  if(!root)return null;
...
async function applyFront(){
  ...
  if(card.dataset.creditLifecycleKey===key&&card.dataset.creditLifecycleDone==='1')return;
...
async function applyHistory(){
  ...
  if(!uid||!panel)return;
  var key=String(state.selectedUnit);
  if(panel.dataset.creditLifecycleKey===key&&panel.dataset.creditLifecycleDone==='1')return;
  panel.dataset.creditLifecycleKey=key;
  try{
    var data=await getState(uid,true);
    if(!window.state||String(state.selectedUnit)!==key||!document.body.contains(panel))return;
    if(data.life.completed){
      ...
      if(row){
        ...
        if(!row.querySelector('.customer-note-archive-help')){
          ...
          help.className='customer-note-archive-help';
          help.textContent=data.life.multi?'Archived after all required installment credit notes were issued.':'Archived after the required credit note was issued.';
          row.appendChild(help);
        }
      }
    }
    panel.dataset.creditLifecycleDone='1';
  }catch(e){console.warn('Could not update credit-note note history',e);}
}
function decorate(){
  applyFront();
  applyHistory();
