...
function decorateNewCustomer(){
  ...
  if(!document.getElementById('partialBookingNoteNew')){
    ...
    wrap.innerHTML='<label class="customer-note-field">Partial Booking Note<textarea id="ncPartialBookingNote" placeholder="e.g. RM advised the remaining Down Payment to be completed by 01-Sep-2026."></textarea></label><p class="customer-note-help">Internal follow-up note only. It does not change the contractual Down Payment due date in the payment schedule.</p>';
    general.closest('label').insertAdjacentElement('afterend',wrap);
    var pt=wrap.querySelector('textarea');
    pt.value=d.partial||'';
    pt.addEventListener('input',function(){draft().partial=pt.value;});
  }
...
async function decorateDetail(){
  ...
  try{
    ...
    card.innerHTML='<p class="section-label">Internal Notes</p>'+
      ...
      '<div id="customerPartialNoteWrap"'+(partialEligible?'':' hidden')+'><label class="customer-note-field">Partial Booking Note<textarea id="customerPartialBookingNote" placeholder="e.g. RM advised the remaining Down Payment to be completed by 01-Sep-2026.">'+safe(data.sale.partial_booking_note||'')+'</textarea></label><p class="customer-note-help">For RM-advised completion follow-up only. The contractual Down Payment schedule date stays unchanged.</p></div>'+
    ...
    saveBtn.addEventListener('click',async function(){
      ...
      try{
        ...
        if(status)status.textContent='Could not save: '+(e&&e.message?e.message:'Unknown error');
      }finally{
