...
window.__sunblissRecordPaymentFixV3=true;

var cache={customer:null,rows:[],credits:{},loading:false,saving:false};
...
function unitId(c){return Number(c&&(c.unitId!=null?c.unitId:c.sno))||0}
...
function closePanel(){
  var p=document.getElementById('recordPaymentReliablePanel');
  if(p)p.remove();
  cache.customer=null;cache.rows=[];cache.credits={};cache.loading=false;cache.saving=false;
...
function setError(msg){
  ...
  if(e){
    ...
    window.setTimeout(function(){try{e.scrollIntoView({block:'center',behavior:'smooth'})}catch(_err){}},0);
  }
  var b=document.getElementById('pfSave');
  if(b&&!cache.saving){b.disabled=false;b.textContent='Save payment'}
}
...
function ensureStyles(){
  ...
  var s=document.createElement('style');s.id='recordPaymentReliableStyles';
  s.textContent=[
...
function toggleCreditFields(){
  var toggle=document.getElementById('pfCreditToggle'),box=document.getElementById('pfCreditFields');
  ...
  if(opening){
    ...
    window.setTimeout(function(){try{box.scrollIntoView({block:'nearest',behavior:'smooth'})}catch(e){}},0);
  }else{
...
function renderForm(){
  ...
  if(rows.length){
    body='<form id="recordPaymentReliableForm" novalidate>'+ 
      ...
      '<p class="record-payment-stage-note">Select the exact payment schedule. The save is processed as one database transaction so the installment ledger and transaction history stay synchronized.</p>'+ 
  ...
  var amount=document.getElementById('pfAmount');if(amount)setTimeout(function(){try{amount.focus()}catch(e){}},0);
}
...
async function openPanel(){
  ...
  try{await loadRows(c);cache.loading=false;renderForm()}
    ...
    cache.loading=false;
    p.innerHTML='<p class="section-label" style="margin-top:0">Record Payment</p><p class="record-payment-summary">Unit '+safe(c.unit||'')+' · '+safe(c.name||'')+'</p><p class="brand-error">'+safe(e&&e.message?e.message:'Could not load the payment schedule.')+'</p><div class="brand-editor-actions"><button class="btn-paper" type="button" id="pfCancel" style="justify-content:center;margin-bottom:0">Close</button></div>';
    var close=document.getElementById('pfCancel');if(close)close.onclick=closePanel;
  }
}
function val(id){var e=document.getElementById(id);return e?text(e.value).trim():''}

async function refreshAfterSave(key,from){
  try{
    if(typeof window.loadFromSupabase==='function')await window.loadFromSupabase();
  }catch(refreshError){
    console.error('[Sunbliss] payment saved but CRM refresh failed',refreshError);
  }
  ...
  }catch(renderError){console.error('[Sunbliss] payment saved but detail refresh failed',renderError)}
}
...
async function savePayment(){
  ...
  try{
    ...
    closePanel();
    await refreshAfterSave(key,from);
  }catch(e){
    cache.saving=false;
    if(window.state)state.paymentFormSaving=false;
    if(btn&&btn.isConnected){btn.disabled=false;btn.textContent='Save payment'}
    setError(e&&e.message?e.message:'Could not save that payment.');
  }
}
...
document.addEventListener('click',function(e){
  ...
  e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openPanel();
},true);
document.addEventListener('click',function(e){
  var toggle=e&&e.target&&e.target.closest?e.target.closest('#recordPaymentReliablePanel #pfCreditToggle'):null;
  if(!toggle)return;
  e.preventDefault();e.stopPropagation();toggleCreditFields();
},true);
