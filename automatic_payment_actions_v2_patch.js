...
async function run(){
 ...
 var byUnit={};
 schedules.forEach(function(r){var sid=String(r.id),kind=stageKind(r);if(!kind||managed[sid])return;var applied=credit[sid]||0,remaining=Math.round(Math.max(0,(Number(r.due_amount)||0)-(Number(r.paid_amount)||0)-applied)*100)/100,due=effectiveDue(r),delta=daysUntil(due);if(remaining<=1000||!due||delta===null)return;(byUnit[String(r.unit_id)]=byUnit[String(r.unit_id)]||[]).push({row:r,kind:kind,remaining:remaining,creditApplied:applied,delta:delta,due:due,extended:!!extended[sid],manual:!!manualSchedule[sid]})});
