...
function manualPaymentTask(r){if(!r||r.status!=='pending'||(r.source&&r.source!=='manual')||r.auto_kind)return false;return /(payment|installment|demand|reminder|outstanding|overdue|receipt|transfer|charges|collection)/.test(norm(text(r.action_label)+' '+text(r.note)))}
...
async function run(){
 ...
 var managed=carryManaged(),manual={},manualSchedule={},activeUnits={};units.forEach(function(x){activeUnits[String(x)]=1});tasks.forEach(function(t){if(!manualPaymentTask(t))return;var ids=t.schedule_id!=null?[String(t.schedule_id)]:idsFromKey(t.auto_key);if(ids.length)ids.forEach(function(id){manualSchedule[String(id)]=1});else manual[String(t.unit_id)]=1});
