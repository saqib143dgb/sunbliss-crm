(function(){
  ...
  function ensureStyles(){
    ...
    style.textContent = [
      ...
      '#customerNotesManagementPanel.crm-full-page-inline .notes-editor-actions{position:sticky!important;bottom:calc(-92px - env(safe-area-inset-bottom))!important;z-index:10!important;margin:16px -28px -11px!important;padding:12px 12px calc(12px + env(safe-area-inset-bottom))!important;background:var(--paper,#F6F1E4)!important;border-top:1px solid var(--paper-line,#DCD2B6)!important;box-shadow:0 -8px 20px rgba(15,26,38,.08)!important}',
  ...
  function markOverlays(){
    ...
    dialogSelectors.forEach(function(selector){
      document.querySelectorAll(selector).forEach(function(dialog){
        dialog.classList.add('crm-full-page-dialog');
      });
    });
  }
