(function(){
  ...
  if (window.__sunblissCancelledForfeitRuleV2Installed) return;
  window.__sunblissCancelledForfeitRuleV2Installed = true;

  var style = document.createElement('style');
  style.id = 'sunblissCancelledForfeitRuleStylesV2';
  style.textContent = [
  ...
  function hideOriginalSettlementControls(settlement, refund, forfeited){
    ...
    if (settlementLabel && settlementLabel.style.display !== 'none') settlementLabel.style.display = 'none';
    ...
    if (refundLabel && forfeitedLabel && refundLabel.parentElement === forfeitedLabel.parentElement){
      if (refundLabel.parentElement.style.display !== 'none') refundLabel.parentElement.style.display = 'none';
    }else{
      if (refundLabel && refundLabel.style.display !== 'none') refundLabel.style.display = 'none';
      if (forfeitedLabel && forfeitedLabel.style.display !== 'none') forfeitedLabel.style.display = 'none';
    }
  }
  ...
  function enforce(panel){
    ...
    if (settlement.value !== 'Forfeited') settlement.value = 'Forfeited';
    if (refund.value !== '0.00') refund.value = '0.00';
    var expected = paid.toFixed(2);
    if (forfeited.value !== expected) forfeited.value = expected;
    refund.dataset.touched = '1';
    forfeited.dataset.touched = '1';
    if (!settlement.disabled) settlement.disabled = true;
    if (!refund.disabled) refund.disabled = true;
    if (!forfeited.disabled) forfeited.disabled = true;
  ...
  if (typeof window.render === 'function' && !window.__sunblissCancelledForfeitRenderWrappedV2){
    ...
    window.__sunblissCancelledForfeitRenderWrappedV2 = true;
  }
