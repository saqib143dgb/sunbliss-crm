...
function hideSpecialNoteFromEditSale(){var f=document.getElementById('scRemarks');if(!f)return;var h=f.closest?f.closest('label.brand-field'):f.parentElement;if(!h)return;h.style.display='none';h.setAttribute('aria-hidden','true');h.dataset.specialNoteMovedToNotes='1';}
async function fetchNote(uid){var r=await sb.from('sales').select('id,remarks,partial_booking_note').eq('unit_id',uid).order('id',{ascending:false}).limit(1);if(r.error)throw r.error;return (r.data||[])[0]||{};}
async function fetchHistory(uid){
  var r=await sb.from('sales_note_history').select('id,old_note,new_note,edited_by,edited_at').eq('unit_id',uid).order('edited_at',{ascending:false}).limit(100);if(r.error)throw r.error;
...
function currentBlock(note,label){var w=document.createElement('div');w.className='notes-current';w.innerHTML='<p class="notes-current-label">'+esc(label||'Current Note')+'</p><p class="notes-current-text">'+esc(txt(note).trim()||'No note recorded for this customer.')+'</p>';return w;}
function renderHistory(section,rows){
  section.innerHTML='<p class="section-label" style="margin:0 0 9px">History</p>';
...
async function showPanel(){
  ...
  var panel=document.createElement('div');panel.id='customerNotesManagementPanel';panel.className='brand-editor';panel.innerHTML='<p class="section-label" style="margin-top:0">Notes</p><p class="stat-sub">Loading note…</p>';var a=anchor(detail);if(a)a.insertAdjacentElement('afterend',panel);else detail.insertBefore(panel,detail.firstChild);
  try{
    var sale=await fetchNote(uid);if(!document.getElementById('customerNotesManagementPanel'))return;panel.innerHTML='<p class="section-label" style="margin-top:0">Notes</p>';panel.appendChild(currentBlock(sale.remarks,'Current Note'));
    if(txt(sale.partial_booking_note).trim())panel.appendChild(currentBlock(sale.partial_booking_note,'Partial Booking Note'));
    var actions=document.createElement('div');actions.className='notes-management-actions';actions.innerHTML='<button type="button" class="notes-management-action" id="notesHistoryBtn">History</button><button type="button" class="notes-management-action" id="notesEditBtn">Add / Edit Existing Note</button>';panel.appendChild(actions);
    ...
    document.getElementById('notesCancelEditBtn').onclick=function(){var ta=document.getElementById('notesEditorText');if(ta)ta.value=txt(sale.remarks);editor.hidden=true;};
    document.getElementById('notesSaveBtn').onclick=async function(){var save=document.getElementById('notesSaveBtn'),ta=document.getElementById('notesEditorText'),err=document.getElementById('notesEditorError');if(!save||!ta)return;save.disabled=true;save.textContent='Saving…';if(err)err.style.display='none';try{var r=await sb.rpc('crm_save_sales_note',{p_unit_id:uid,p_note:ta.value});if(r.error)throw r.error;sale.remarks=ta.value.trim()||null;var cur=panel.querySelector('.notes-current');if(cur)cur.replaceWith(currentBlock(sale.remarks,'Current Note'));editor.hidden=true;history.hidden=false;renderHistory(history,await fetchHistory(uid));var card=document.getElementById('customerNotesCard');if(card)card.remove();var d=document.querySelector('.detail');if(d)delete d.dataset.customerNotesLoading;if(typeof window.renderDetail==='function')setTimeout(function(){try{window.renderDetail();}catch(e){}},30);}catch(e){if(err){err.textContent=e&&e.message?e.message:'Could not save note.';err.style.display='block';}}finally{if(save){save.disabled=false;save.textContent='Save note';}}};
...
function refresh(){styles();hideSpecialNoteFromEditSale();}
function install(){if(!window.state||!window.sb){setTimeout(install,80);return;}document.addEventListener('click',intercept,true);var root=document.getElementById('app')||document.body;new MutationObserver(function(){requestAnimationFrame(refresh);}).observe(root,{childList:true,subtree:true});refresh();}
install();
})();
