(function(){
  ...
  style.textContent = [
    ...
    '#sunblissPersistentBack{position:fixed;z-index:1298;left:50%;bottom:calc(10px + env(safe-area-inset-bottom));display:none;align-items:center;justify-content:center;gap:7px;width:min(600px,calc(100vw - 20px));height:72px;padding:0 18px;box-sizing:border-box;border:1px solid rgba(255,255,255,.86);border-radius:999px;background:rgba(246,241,228,.94);color:var(--ink-2);font:650 12.5px/1 Inter,system-ui,sans-serif;box-shadow:0 10px 30px rgba(15,26,38,.24),inset 0 1px 0 rgba(255,255,255,.72);-webkit-backdrop-filter:blur(20px) saturate(1.18);backdrop-filter:blur(20px) saturate(1.18);cursor:pointer;touch-action:manipulation;-webkit-tap-highlight-color:transparent;transform:translateX(-50%);transition:background .16s ease,box-shadow .16s ease,transform .12s ease;}',
    ...
    '#sunblissPersistentBack:hover{background:rgba(246,241,228,.99);box-shadow:0 11px 32px rgba(15,26,38,.26),inset 0 1px 0 rgba(255,255,255,.8);}',
    '#sunblissPersistentBack:active{transform:translateX(-50%) scale(.985);}',
    '#sunblissPersistentBack:focus-visible{outline:2px solid var(--gold-deep);outline-offset:3px;}',
    '@media(max-width:420px){#sunblissPersistentBack{bottom:calc(8px + env(safe-area-inset-bottom));width:calc(100vw - 16px);height:70px;padding:0 14px;font-size:12px;}}'
  ].join('');
