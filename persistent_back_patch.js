(function(){
  ...
  style.textContent = [
    '#app #btnBack,#app button.back,#app a.back,body .sunbliss-inline-back-source{display:none!important;}',
    ...
    '#sunblissPersistentBack{position:fixed;z-index:10050;left:50%;bottom:calc(10px + env(safe-area-inset-bottom));display:none;align-items:center;justify-content:center;gap:6px;width:min(600px,calc(100vw - 20px));height:56px;padding:0 16px;box-sizing:border-box;border:1px solid rgba(255,255,255,.62);border-radius:999px;background:rgba(246,241,228,.50);color:var(--ink-2);font:650 12px/1 Inter,system-ui,sans-serif;box-shadow:0 7px 22px rgba(15,26,38,.16),inset 0 1px 0 rgba(255,255,255,.52);-webkit-backdrop-filter:blur(22px) saturate(1.16);backdrop-filter:blur(22px) saturate(1.16);cursor:pointer;touch-action:manipulation;-webkit-tap-highlight-color:transparent;transform:translateX(-50%);transition:background .16s ease,box-shadow .16s ease,transform .12s ease;}',
  ...
  window.addEventListener('pageshow',sync);window.addEventListener('popstate',sync);window.addEventListener('resize',sync);
  sync();
})();
