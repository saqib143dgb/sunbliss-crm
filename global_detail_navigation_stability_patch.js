(function(){
  ...
  function repairShell(){
    ...
    try{
      ...
      console.error('Could not repair CRM shell before detail navigation',err);
    }finally{
      repairing=false;
    }
    return currentMain();
  ...
  function activeCustomers(){
    return window.state && Array.isArray(state.dues) ? state.dues.filter(Boolean) : [];
  }
  ...
  function resolveCustomer(unit,sno,source){
    ...
    var exact=rows.filter(function(c){
      return text(c.sno).trim()===idKey && norm(c.unit)===unitKey;
    });
  ...
  function customerKey(customer){
    return text(customer.unit)+'::'+text(customer.sno);
  }
  ...
  var observer=new MutationObserver(scheduleVerify);
  observer.observe(document.documentElement,{childList:true,subtree:true});
  window.addEventListener('pageshow',scheduleVerify);
  window.addEventListener('popstate',scheduleVerify);

  window.__sunblissOpenCustomerDetail=function(unit,sno,from){
    return canonicalGoToDetail(unit,sno,from,null);
  };
