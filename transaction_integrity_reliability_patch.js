...
function installStyles(){
  ...
  var s=document.createElement('style');s.id='transactionIntegrityReliabilityStyles';s.textContent=[
    '#transactionEditPanel{max-width:100%!important;box-sizing:border-box!important;overflow-x:hidden!important;margin:8px 0 14px!important}',
...
function detailIntegrity(){
  ...
  if(name&&norm(name.textContent)!==norm(typeof window.titleCase==='function'?window.titleCase(c.name):c.name))return{ok:false,reason:'customer mismatch'};
  ...
  var txs=exactTransactions(c),rows=detail.querySelectorAll('.tx-list .tx-row');
  if(rows.length!==txs.length)return{ok:false,reason:'transaction mismatch'};
  return{ok:true};
}
function closeMenus(){document.querySelectorAll('.tx-actions-menu').forEach(function(m){m.style.display='none'});document.querySelectorAll('.tx-actions-btn').forEach(function(b){b.setAttribute('aria-expanded','false')})}
function decorateTransactionRows(){
  ...
  var txs=exactTransactions(c),rows=detail.querySelectorAll('.tx-list .tx-row');
  rows.forEach(function(row,index){var t=txs[index];if(t&&t.id!=null){row.dataset.transactionId=text(t.id);row.dataset.transactionUnitId=text(t.unitId||t.unit_id||customerUnitId(c));row.dataset.transactionCustomerId=text(t.customerId||t.customer_id||customerId(c))}else{delete row.dataset.transactionId;delete row.dataset.transactionUnitId;delete row.dataset.transactionCustomerId}});
...
function repairDetail(reason){
  if(repairing||!window.state||state.view!=='detail'||!state.selectedUnit||typeof previousRenderDetail!=='function')return false;
  var key=text(state.selectedUnit),count=repairAttempts[key]||0;if(count>=3)return false;repairAttempts[key]=count+1;
  repairing=true;
  try{
...
function verifyNow(){
  ...
  if(!check.ok&&repairDetail(check.reason)){
    setTimeout(function(){decorateTransactionRows();var again=detailIntegrity();if(!again.ok)repairDetail(again.reason)},40);
...
function currentRowIndex(row){var rows=Array.prototype.slice.call(document.querySelectorAll('#main .detail .tx-list .tx-row'));return rows.indexOf(row)}
async function fetchTransactionForRow(row){
  ...
  var tx=null;
  if(id){var one=await sb.from('payment_transactions').select('*').eq('id',id).single();if(one.error)throw one.error;tx=one.data}
  else{
    ...
    var list=await q;if(list.error)throw list.error;var index=currentRowIndex(row);tx=(list.data||[])[index]||null;
  }
  if(!tx)throw new Error('Transaction not found. The page has been refreshed for safety.');
  if(text(state.selectedUnit)!==key)throw new Error('The customer changed while the transaction was loading. Please try again.');
  if(uid&&Number(tx.unit_id)!==uid)throw new Error('This transaction belongs to a different unit. The page has been refreshed for safety.');
  if(cid&&Number(tx.customer_id)!==cid)throw new Error('This transaction belongs to a different customer. The page has been refreshed for safety.');
  return tx;
}
function editorInput(id,label,val,type,extra){return '<label class="brand-field">'+safe(label)+'<input id="'+id+'" type="'+(type||'text')+'" value="'+safe(val==null?'':val)+'"'+(extra||'')+' /></label>'}
async function openReliableEditor(row){
  closeMenus();var old=document.getElementById('transactionEditPanel');if(old)old.remove();
  var c=currentCustomer();if(!c)return;
  var key=text(state.selectedUnit),panel=document.createElement('div');panel.id='transactionEditPanel';panel.className='brand-editor';panel.innerHTML='<p class="section-label" style="margin-top:0">Edit transaction</p><p class="stat-sub">Loading transaction…</p>';
  row.insertAdjacentElement('afterend',panel);
  try{
    var tx=await fetchTransactionForRow(row);
    var schedules=await sb.from('payment_schedule').select('id,customer_id,unit_id,stage_name,due_amount,paid_amount,due_date').eq('customer_id',tx.customer_id).eq('unit_id',tx.unit_id).order('due_date',{ascending:true,nullsFirst:false}).order('id',{ascending:true});
    if(schedules.error)throw schedules.error;if(text(state.selectedUnit)!==key)throw new Error('The customer changed while the editor was opening.');
    var rows=schedules.data||[];if(!rows.length)throw new Error('No installment schedule is linked to this customer and unit.');
    var selectedId=Number(tx.payment_schedule_id)||0;if(!selectedId){var base=paymentBase(tx.payment_type),match=rows.find(function(s){return paymentBase(s.stage_name)===base});selectedId=match?Number(match.id):0}
    var opts=rows.map(function(s){return '<option value="'+safe(s.id)+'"'+(Number(s.id)===selectedId?' selected':'')+'>'+safe(s.stage_name)+' · '+safe(money(Math.max(0,num(s.due_amount)-num(s.paid_amount))))+' remaining</option>'}).join('');
    panel.innerHTML='<p class="section-label" style="margin-top:0">Edit transaction</p><p class="stat-sub" style="margin:-5px 0 12px">Edit the payment safely. The installment ledger is rebalanced automatically and the change remains in the audit log.</p><p class="brand-error" id="transactionEditError" style="display:none"></p><label class="brand-field">Installment<select id="teStage">'+opts+'</select></label>'+editorInput('teAmount','Amount paid (AED)',tx.amount,'number',' min="0.01" step="0.01" inputmode="decimal"')+editorInput('teDate','Payment date',isoDate(tx.payment_date),'date')+editorInput('teRef','Reference (optional)',tx.payment_reference||'')+editorInput('teRemarks','Remarks (optional)',tx.remarks||'')+'<div class="brand-editor-actions"><button type="button" class="btn btn-gold" id="teSave" style="justify-content:center">Save changes</button><button type="button" class="btn-paper" id="teCancel" style="justify-content:center;margin-bottom:0">Cancel</button></div>';
    ...
    panel.innerHTML='<p class="section-label" style="margin-top:0">Edit transaction</p><p class="brand-error">'+safe(err&&err.message?err.message:'Could not load that transaction.')+'</p><button type="button" class="btn-paper" id="teCloseError">Close</button>';
    var close=document.getElementById('teCloseError');if(close)close.onclick=function(){panel.remove();scheduleVerify()};
    repairDetail('transaction editor identity check');
  }
}
...
async function saveReliableEdit(){
  if(!activeEdit)return;var ctx=activeEdit,err=document.getElementById('transactionEditError'),btn=document.getElementById('teSave');
  var c=currentCustomer();if(!c||text(state.selectedUnit)!==ctx.key||customerUnitId(c)!==Number(ctx.tx.unit_id)){if(err){err.textContent='The displayed customer changed. Close this editor and try again.';err.style.display='block'}return}
  ...
  if(!stage){err.textContent='Select an installment.';err.style.display='block';return}if(!isFinite(amount)||amount<=0){err.textContent='Enter a valid payment amount greater than zero.';err.style.display='block';return}if(!date){err.textContent='Select a payment date.';err.style.display='block';return}
  btn.disabled=true;btn.textContent='Saving…';err.style.display='none';
  var from=state.detailFrom||'list',key=ctx.key;
  try{
    var r=await sb.rpc('crm_edit_payment_transaction',{p_transaction_id:ctx.tx.id,p_payment_date:date,p_amount:Math.round(amount*100)/100,p_payment_type:stage.stage_name,p_payment_reference:value('teRef')||null,p_remarks:value('teRemarks')||null});
    if(r.error)throw r.error;activeEdit=null;if(ctx.panel&&ctx.panel.isConnected)ctx.panel.remove();await refreshBackToCurrent(key,from);
  }catch(e){if(err){err.textContent=e&&e.message?e.message:'Could not save that transaction.';err.style.display='block'}btn.disabled=false;btn.textContent='Save changes'}
}
async function deleteReliableTransaction(row,button){
  closeMenus();var c=currentCustomer();if(!c)return;var key=text(state.selectedUnit),from=state.detailFrom||'list';
  try{
    var tx=await fetchTransactionForRow(row);
    ...
    var r=await sb.rpc('crm_delete_payment_transaction',{p_transaction_id:tx.id});if(r.error)throw r.error;
    activeEdit=null;var p=document.getElementById('transactionEditPanel');if(p)p.remove();await refreshBackToCurrent(key,from);
  }catch(e){window.alert(e&&e.message?e.message:'Could not delete that transaction.');if(button){button.disabled=false;button.textContent='Delete transaction'}repairDetail('transaction delete identity check')}
}

function captureTransactionAction(event){
  var target=event.target&&event.target.closest?event.target.closest('.tx-actions-menu button'):null;if(!target)return;
  var label=norm(target.textContent);if(label!=='edit transaction'&&label!=='delete transaction')return;
  var row=target.closest('.tx-row');if(!row)return;
  event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();
  if(label==='edit transaction')openReliableEditor(row);else deleteReliableTransaction(row,target);
}
...
function wrapFunctions(){
  ...
  if(typeof previousRenderDetail==='function'&&!previousRenderDetail.__sunblissTransactionIntegrityWrapped){
    var rd=function(){var key=window.state?text(state.selectedUnit):'';var out=previousRenderDetail.apply(this,arguments);if(window.state&&state.view==='detail'&&text(state.selectedUnit)===key){decorateTransactionRows();scheduleVerify()}return out};
  ...
  if(typeof previousRenderMain==='function'&&!previousRenderMain.__sunblissTransactionIntegrityWrapped){
    var rm=function(){var out=previousRenderMain.apply(this,arguments);if(window.state&&state.view==='detail'){decorateTransactionRows();scheduleVerify()}return out};rm.__sunblissTransactionIntegrityWrapped=true;window.renderMain=rm;
  }
  ...
  if(typeof previousGoToDetail==='function'&&!previousGoToDetail.__sunblissTransactionIntegrityWrapped){
    var gd=function(){var out=previousGoToDetail.apply(this,arguments);scheduleVerify();setTimeout(scheduleVerify,90);return out};gd.__sunblissTransactionIntegrityWrapped=true;window.goToDetail=gd;
  }
  previousLoad=window.loadFromSupabase;
  if(typeof previousLoad==='function'&&!previousLoad.__sunblissTransactionIntegrityWrapped){
    var ld=async function(){var out=await previousLoad.apply(this,arguments);installExactMatcher();scheduleVerify();return out};ld.__sunblissTransactionIntegrityWrapped=true;window.loadFromSupabase=ld;
  }
}
function install(){
  ...
  installStyles();wrapFunctions();document.addEventListener('click',captureTransactionAction,true);
  if(window.MutationObserver){var main=document.getElementById('main');if(main)new MutationObserver(function(){if(window.state&&state.view==='detail')scheduleVerify()}).observe(main,{childList:true,subtree:true})}
  window.addEventListener('pageshow',scheduleVerify);window.addEventListener('popstate',scheduleVerify);scheduleVerify();
}
install();
})();
